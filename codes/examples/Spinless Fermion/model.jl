...
function ckMPO(Latt::AbstractLattice,k::Vector)
    return compress(canonicalize(KOprMPO(LocalSpace.F,Latt,k,"Fk",LocalSpace.Z)))
end

function ckdagMPO(Latt::AbstractLattice,k::Vector)
    return compress(canonicalize(KOprMPO(LocalSpace.Fdag,Latt,k,"Fkdag",LocalSpace.Z)))
end
