...
function contract(El::LeftCompositeEnvironmentTensor{2, 4, 3, 1}, Er::RightEnvironmentTensor{2})
    @tensor tmp[-1 -2;-3 -4] ≔ El.A[1,-2,-3,-4] * Er.A[-1,1]
    return AdjointMPOTensor(tmp)
end
...
function contract(El::LeftEnvironmentTensor{2}, Er::RightCompositeEnvironmentTensor{2, 4, 3, 1})
    @tensor tmp[-1 -2;-3 -4] ≔ El.A[1,-4] * Er.A[-1,-2,1,-3]
    return AdjointMPOTensor(tmp)
end
