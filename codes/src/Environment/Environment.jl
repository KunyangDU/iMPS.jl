...
function RightLsEnv(
    ...
    LR = length(ψ) + 1 - site
    lsEnvR = Vector{AbstractTensorMap{ComplexSpace,1,2}}(undef,LR)

    lsEnvR[LR] = InitialEnv(lsEnvR;ds = [dim(domain(ψ[end])[2]), dim(codomain(H[end])[1]),dim(domain(ψ[end])[2])])
    for i in LR-1:-1:1
...
function LeftLsEnv(ψ::Vector,H::Vector,site::Int64)
    ...
    lsEnvL[1] = InitialEnv(lsEnvL;ds = [dim(domain(ψ[1])[1]), dim(domain(H[1])[1]),dim(domain(ψ[1])[1])])
    for i in 2:LL
