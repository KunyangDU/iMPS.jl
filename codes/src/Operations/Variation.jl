...
function VariContract(Opr::Vector{Union{AbstractTensorMap{ComplexSpace,2,2},AbstractTensorMap{ComplexSpace,1,3}}},
    ψ₀::Vector{Union{AbstractTensorMap{ComplexSpace,1,2},AbstractTensorMap{ComplexSpace,0,3}}},
    D_MPS::Int64;MaxIter::Int64=4)

    L = length(Opr)
    d = dim(codomain(Opr[1]))
    ψtest = RandMPS(L;d=d)
    ...
    for iter in 1:MaxIter
        ...
        println(">>>>>> begin >>>>>>")
        for i in 1:L-1
            Ev = EnvMerge(lsEnv[i],MPS[i:i+1]...,Opr[i:i+1]...,lsEnv[i+2])
            ψtest[i:i+1],temptruncerr_test = RightSVD(Ev,D_MPS)
            MPS[i:i+1],temptruncerr_MPS = RightMove(MPS[i+1],MPS[i],D_MPS)
            Opr[i:i+1] = Move(Opr[i:i+1]...)
            lsEnv[i+1] = PushRight(lsEnv[i],MPS[i],Opr[i],ψtest[i])
            totaltruncerror = max(totaltruncerror,temptruncerr_test,temptruncerr_MPS)
        end 
        println(">>>>>> finished >>>>>>")        
        ...
        println("<<<<<< begin <<<<<<")
        for i in L:-1:2
            Ev = EnvMerge(lsEnv[i-1],MPS[i-1:i]...,Opr[i-1:i]...,lsEnv[i+1])
            ψtest[i-1:i],temptruncerr_test = LeftSVD(Ev,D_MPS)
            MPS[i-1:i],temptruncerr_MPS = LeftMove(MPS[i-1],MPS[i],D_MPS)
...
function VariPlusMPO(
    ...
    for iter in 1:MaxIter
        ...
        println(">>>>>> begin >>>>>>")
        for i in 1:L-1
            localOpr = EnvMerge(lsEnv1[i],tempOpr1[i:i+1]...,lsEnv1[i+2]) + EnvMerge(lsEnv2[i],tempOpr2[i:i+1]...,lsEnv2[i+2])
        ...
        println("<<<<<< begin <<<<<<")
        for i in L:-1:2
            localOpr = EnvMerge(lsEnv1[i-1],tempOpr1[i-1:i]...,lsEnv1[i+1]) + EnvMerge(lsEnv2[i-1],tempOpr2[i-1:i]...,lsEnv2[i+1])
...
function VariProdMPO(
    ...
    for iter in 1:MaxIter
        ...
        println(">>>>>> begin >>>>>>")
        for i in 1:L-1
            localOpr = EnvMerge(lsEnv[i],tempOpr1[i:i+1]...,tempOpr2[i:i+1]...,lsEnv[i+2])
            testOpr[i:i+1],temptruncerr_test = mySVD(localOpr,"right",D_MPO)
        ...
        println("<<<<<< begin <<<<<<")
        for i in L:-1:2
            localOpr = EnvMerge(lsEnv[i-1],tempOpr1[i-1:i]...,tempOpr2[i-1:i]...,lsEnv[i+1])
            testOpr[i-1:i],temptruncerr_test = mySVD(localOpr,"left",D_MPO)
