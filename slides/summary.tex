...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{figure}[H]
			...
			\subfigure{\includegraphics[height=0.3\linewidth]{images/χ_D=32_4x4.pdf}}
		\end{figure}
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Obserables: Spectrum function}
	\begin{itemize}
		\item Spectrum function
		\begin{gather}
			S(\vb*{k},E) = -\frac{1}{\pi}\mathrm{Im} G^{ret}(\vb*{k},E)
		\end{gather}
		...
		\begin{equation}
			\begin{gathered}
				...
				\begin{aligned}
					...
					=&\begin{cases}
						&-i\mathrm{e}^{iE_0t}\left(\bra{\Psi_0}c_k\right)\mathrm{e}^{-iHt}\left(c_k^\dagger\ket{\Psi_0}\right) \text{ if } t >0\\
						&-i\mathrm{e}^{-iE_0t}\left(\bra{\Psi_0}c_k^\dagger\right)\mathrm{e}^{iHt}\left(c_k\ket{\Psi_0}\right) \text{ if } t <0
					  \end{cases}
				\end{aligned}
			\end{gathered}
		\end{equation}
...
\begin{frame}
	...
	\begin{align}
		\left(\bra{\Psi_0}c_i\right)\mathrm{e}^{-iHt}\left(c_j^\dagger\ket{\Psi_0}\right)\\
		\left(\bra{\Psi_0}c_i^\dagger\right)\mathrm{e}^{iHt}\left(c_j\ket{\Psi_0}\right)
	\end{align}
	i.e. calculate the $2L$ MPO*MPS, then TDVP by all-to-all inner product. 
\end{frame}
